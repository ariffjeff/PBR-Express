...
def main(kwargs):
    ...
    def getTextureSet():
        userInput = hou.ui.selectFile(
            title=("Choose one of the textures or input the folder path."),
    ...
    def textureFinder(inputFiles):
        ...
        for file in input_files_clean:
            ...
            if (
                name.split("_")[-1] in resolutions
            ):  # If the last word after the sepparator is a resolution from the list above, fall back to the 2nd word
        ...
        if duplicates:
            ...
            hou.ui.displayMessage(
                "Seems like you have selected duplicate file endings (multiple normal maps for example).",
                title=("BZZ... WRONG"),
            )
            exit()
        ...
        if len(invalid_fileTypes) != 0:
            print(
                f"[ERROR] Those files types are not supported image files: {invalid_fileTypes}"
            )
            print(f"[ERROR] List of supported image files: {str(validFileTypes())}")
            hou.ui.displayMessage(
                "Seems like one of your files is not an image file, check the console for more info.",
                title=("BZZ... WRONG"),
            )
            exit()
    ...
    def metadataAssign(old_data):
        ...
        for file in quick_list:
            ...
            if (
                t == None
            ):  # Checking so we can have multiple files that are of the textureType "None"
                pass
            else:
                if t in unique_type:
                    print(f"[ERROR] Multiple files of the same textureType detected: {t}")
                    hou.ui.displayMessage(
                        f"Seems like you have selected multiple files of the same textureType, check the console for more info",
                        title=("BZZ... WRONG"),
                    )
                    exit()
                else:
    ...
    def renderHandler(renderer_names):
        ...
        render_selection = hou.ui.selectFromList(
            renderer_names,
            exclusive=True,
            title=("Render Handler"),
            message=("For which renderer should the material be created?"),
            column_header="Renderers",
            width=500,
            height=200,
        )
    ...
    def goalSelection():
        ...
        if currentPane_type == "Vop":
            ...
            print(
                f"[SUCCESS] A valid destination for the nodes has been selected: {goalPath}"
            )
            ...
            if goalType != "Vop":
                print(
                    f"[ERROR] The selected destination is not valid for material nodes: {goalPath}"
                )
                hou.ui.displayMessage(
                    "The selected node destination can't be used for materials, check the console for more info.",
                    title=("BZZ... WRONG"),
                )
                exit()
    ...
    def nodeCreation(renderer, goal, file_data):
        ...
        if len(file_data) == len(faulty_data):
            ...
            hou.ui.displayMessage(
                "Seems like none of the selected files could be matched to any predetermined or custom texture types. See console for info."
            )
            exit()
        ...
        if len(faulty_data) != 0:
            ...
            selection = hou.ui.displayMessage(
                "Seems like you selected some files could not be matched to any predetermined or custom texture types, should the texture nodes be created anyway? (See console for info)",
                buttons=("Create anyway", "Forget them"),
            )
        ...
        if renderer == "Karma":
            ...
            newParm_hidingFolder = hou.FolderParmTemplate(
                "mtlxBuilder", "MaterialX Builder", folder_type=hou.folderType.Collapsible
            )
            ...
            newParam_uvScale = hou.FloatParmTemplate(
                "uvscale", "UV Scale", 2, default_value=(1, 1)
            )
            newParam_uvOffset = hou.FloatParmTemplate(
                "uvoffset", "UV Offset", 2, default_value=(0, 0)
            )
            newParam_uvRotate = hou.FloatParmTemplate("uvrotate", "UV Rotate", 1)
            newParam_separator = hou.SeparatorParmTemplate("separator")
            newParam_displacement = hou.FloatParmTemplate(
                "displacement", "Displacement", 1, default_value=(0.05, 0)
            )
            ...
            subnet_output_disp = goalNode.createNode(
                "subnetconnector", "displacement_output"
            )
            ...
            for metadata in file_data:
                ...
                if textureType == "ROUGH":
                    MTLX_StSf_Node.setNamedInput(
                        "specular_roughness", MTLX_Image_Node, "out"
                    )
                    MTLX_Image_Node.parm("signature").set("float")
            ...
            if (
                "DIFFUSE" not in detected_texture_types
                and "AO" not in detected_texture_types
            ):
            ...
            print(
                f"[SUCCESS] All MTLX nodes for texture set '{set_name}' have been created."
            )
            goalNode.layoutChildren()

        if (
            renderer == "Mantra"
        ):  # I am aware that this implementations is pretty basic, but since Karma seems to be taking over I assume that most people will use MTLX anyway
            ...
            print(
                f"[SUCCESS] All Mantra nodes for texture set '{set_name}' have been created."
            )
    ...
    if kwargs["shiftclick"] or kwargs["ctrlclick"]:
        ...
        print(
            "[INFO] Bulk Textures setup has been initiated through the press of SHIFT / CNTRL."
        )
        ...
        for tex_set in input_texture_sets:
            set_name = os.path.splitext(os.path.basename(tex_set.split(" ; ")[0]))[
                0
            ].rsplit("_", 1)[0]
            file_data = textureFinder(tex_set)
        ...
        input_files = getTextureSet()
        set_name = os.path.splitext(os.path.basename(input_files.split(" ; ")[0]))[
            0
        ].rsplit("_", 1)[0]
